...
`
    ...
    const fethc_courses= async () =>{
        try{

            const res=await fetch(`http://127.0.0.1:5000/courses-with-id`,{
                method:"GET"
            })
            return res.json()
        }
    ...
    courses.innerHTML=''
    const response=await fethc_courses()
    const crses=response['data']
    ...
    const fetch_add_video=async (payload) =>{
        try{
            const res= await fetch(`http://127.0.0.1:5000/add-video`,{
                method:"POST",
                body:payload,
                headers:{
                    "token":sessionStorage.getItem("token")
    ...
    const frm=document.getElementById('add-video-form')
    frm.addEventListener('submit', (event)=>{
        event.preventDefault();
        const add_frm=event.target
        const payload=new FormData(add_frm)
        fetch_add_video(payload)
    })
})